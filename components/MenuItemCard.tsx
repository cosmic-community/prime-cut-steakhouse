...
export default function MenuItemCard({ item, featured = false }: MenuItemCardProps) {
  const dishPhoto = item.metadata?.dish_photo
  const winePairing = item.metadata?.wine_pairing
  const isSpecial = item.metadata?.chefs_special
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-400">
                  {winePairing.metadata.vineyard} • {winePairing.metadata.year}
                </p>
